...
draw_empty_deck_test() ->
  {Deck, Hand} = {[], [{blue, rio}]},
  ?assertEqual({game_over, empty_player_deck}, card:draw(Deck, Hand)).

draw_normal_card_deck_test() ->
  {Deck, Hand} = {[{red, ny}, {black, delhi}], [{blue, rio}]},
  ?assertEqual({ok, [{black, delhi}], [{red, ny}, {blue, rio}]}, card:draw(Deck, Hand)).

draw_invasion_card_deck_test() ->
  {Deck, Hand} = {[invasion, {black, delhi}], [{blue, rio}]},
  ?assertEqual({invasion, [{black, delhi}], [{blue, rio}]}, card:draw(Deck, Hand)).
...
reveal_normal_deck_test() ->
  ...
  ?assertEqual({{red, ny}, [{black, delhi}], [{red, ny}, {blue, rio}]}, card:reveal(Deck, Pile)).
...
discard_card_in_hand_test() ->
  ...
  ?assertEqual({[{blue, rio}], [{red, ny}, {black, delhi}]}, card:discard(Card, Hand, Discard)).
...
initial_hand_for_two_players_test() ->
  ...
  P1 = [1,3,5,7],
  P2 = [2,4,6,8],
  ?assertEqual({NewDeck, [P1, P2]}, card:initial_hand(PlayerDeck, 2)).
...
initial_hand_for_three_players_test() ->
  ...
  P2 = [2,5,8],
  P3 = [3,6,9],
  ?assertEqual({NewDeck, [P1, P2, P3]}, card:initial_hand(PlayerDeck, 3)).
...
initial_hand_for_four_players_test() ->
  ...
  P3 = [3,7],
  P4 = [4,8],
  ?assertEqual({NewDeck, [P1, P2, P3, P4]}, card:initial_hand(PlayerDeck, 4)).
...
initial_hand_for_five_players_test() ->
  ...
  P4 = [4,9],
  P5 = [5,10],
  ?assertEqual({NewDeck, [P1, P2, P3, P4, P5]}, card:initial_hand(PlayerDeck, 5)).
